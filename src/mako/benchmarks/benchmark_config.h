...
struct ShardContext {
  ...
  rusty::Arc<SiloRuntime>
      runtime;              // Per-shard Silo runtime (epoch, threads, core IDs)
  ...
  std::unordered_map<uint16_t, mako::HelperQueue *>
      queue_holders; // Request queues
  std::unordered_map<uint16_t, mako::HelperQueue *>
      queue_holders_response;                          // Response queues
  std::map<int, abstract_ordered_index *> open_tables; // Per-shard tables

  ShardContext()
      : shard_index(-1), runtime(nullptr), db(nullptr), transport(nullptr) {}
};
...
private:
  ...
  BenchmarkConfig()
      : nthreads_(1), num_erpc_server_(2), // number of erpc pull threads
        scale_factor_(1.0), nshards_(1),   // default 1 shard
        shardIndex_(0),                    // default on the shard-0
        cluster_("localhost"), clusterRole_(0), config_(nullptr),
        running_(true), control_mode_(0), verbose_(1), txn_flags_(1),
        runtime_(30), runtime_plus_(0), ops_per_worker_(0),
        run_mode_(RUNMODE_TIME), enable_parallel_loading_(0), pin_cpus_(1),
        slow_exit_(0), retry_aborted_transaction_(1), no_reset_counters_(0),
        backoff_aborted_transaction_(0), use_hashtable_(0),
        is_micro_(0), // if run micro-based workload
        end_received_(0), end_received_leader_(0), replay_batch_(0),
  ...
  std::string nfs_sync_dir_{
      "/tmp/mako_sync"};         // Shared directory for sync files
  int nfs_sync_timeout_sec_{60}; // Timeout for waiting for other shards

  // Cross-shard transaction control (TPC-C)
  int new_order_remote_item_pct_{-1}; // -1 means use tpcc.cc default
...
public:
  ...
  int getBackoffAbortedTransaction() const {
    return backoff_aborted_transaction_;
  }
  // @safe
  ...
  // @safe
  int getLeaderConfig() const {
    return paxos_proc_name_ == mako::LOCALHOST_CENTER;
  }
  const std::vector<std::string> &getPaxosConfigFile() const {
    return paxos_config_file_;
  }

  // Runtime TPCC wiring getters
  std::vector<FastTransport *> &getServerTransports() {
    return server_transports_;
  }
  const std::vector<FastTransport *> &getServerTransports() const {
    return server_transports_;
  }
  std::unordered_map<uint16_t, mako::HelperQueue *> &getQueueHolders() {
    return queue_holders_;
  }
  const std::unordered_map<uint16_t, mako::HelperQueue *> &
  getQueueHolders() const {
    return queue_holders_;
  }
  std::unordered_map<uint16_t, mako::HelperQueue *> &getQueueHoldersResponse() {
    return queue_holders_response_;
  }
  const std::unordered_map<uint16_t, mako::HelperQueue *> &
  getQueueHoldersResponse() const {
    return queue_holders_response_;
  }
  std::atomic<int> &getServerTransportReadyCounter() {
    return set_server_transport_;
  }
  const std::atomic<int> &getServerTransportReadyCounter() const {
    return set_server_transport_;
  }
  ...
  void setEnableParallelLoading(int enable) {
    enable_parallel_loading_ = enable;
  }
  void setPinCpus(int pin) { pin_cpus_ = pin; }
  void setSlowExit(int slow) { slow_exit_ = slow; }
  void setRetryAbortedTransaction(int retry) {
    retry_aborted_transaction_ = retry;
  }
  void setNoResetCounters(int no_reset) { no_reset_counters_ = no_reset; }
  void setBackoffAbortedTransaction(int backoff) {
    backoff_aborted_transaction_ = backoff;
  }
  void setUseHashtable(int use) { use_hashtable_ = use; }
  void setIsMicro(int micro) { is_micro_ = micro; }
  void setNewOrderRemoteItemPct(int pct) { new_order_remote_item_pct_ = pct; }
  int getNewOrderRemoteItemPct() const { return new_order_remote_item_pct_; }
  void setIsReplicated(int replicated) { is_replicated_ = replicated; }
  void setPaxosProcName(std::string paxos_proc_name) {
    paxos_proc_name_ = paxos_proc_name;
    setCluster(paxos_proc_name);
    setClusterRole(mako::convertCluster(paxos_proc_name));
  }
  void setPaxosConfigFile(const std::vector<std::string> &paxos_config_file) {
    paxos_config_file_ = paxos_config_file;
  }
  ...
  std::vector<std::pair<uint32_t, uint32_t>> &getAdvanceWatermarkTracker() {
    return advanceWatermarkTracker_;
  }
  const std::vector<std::pair<uint32_t, uint32_t>> &
  getAdvanceWatermarkTracker() const {
    return advanceWatermarkTracker_;
  }

  // Multi-shard context management
  std::map<int, ShardContext> &getShardContexts() { return shard_contexts_; }
  const std::map<int, ShardContext> &getShardContexts() const {
  ...
  bool hasMultipleShards() const { return shard_contexts_.size() > 1; }
