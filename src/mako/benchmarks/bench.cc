...
extern "C" void malloc_stats_print(void (*write_cb)(void *, const char *),
...
template <typename T> static void delete_pointers(const vector<T *> &pts) {
...
static pair<uint64_t, uint64_t> get_system_memory_info() {
  struct sysinfo inf;
  sysinfo(&inf);
  return make_pair(inf.mem_unit * inf.freeram, inf.mem_unit * inf.totalram);
}

static bool clear_file(const char *name) {
  ofstream ofs(name);
  ofs.close();
  return true;
}
...
static void write_cb(void *p, const char *s) UNUSED;
static void write_cb(void *p, const char *s) {
...
void bench_worker::run() {
  ...
  register_fasttransport_for_bench([&](int control, int value) {
    Warning("receive a control in register_fasttransport_for_bench: %d, "
            "EpochInms: %llu",
            control, getEpochInms());
    switch (control) {
    ...
    case 0: {
      ...
      for (int par_id = 0; par_id < benchConfig.getNthreads(); par_id++) {
        ...
        if (it != shardClientAll.end() && it->second != nullptr) {
          ...
          Warning(
              "ShardClient for par_id=%d is nullptr in setBlocking, skipping",
              par_id);
        }
      }
      break;
    ...
    case 1: {
      ...
      for (int i = 0; i < benchConfig.getNshards(); i++) {
        ...
        std::string w_i = mako::NFSSync::get_key(
        ...
        std::cout << "get fvw, " << clusterRoleLocal
                  << ", fvw_" + std::to_string(i) << ":" << w_i << std::endl;
        fvw[i] = std::stoi(w_i);
      }

      sync_util::sync_logger::update_stable_timestamp_vec(get_epoch() - 1, fvw);
      benchConfig.setControlMode(5); // We can move to the next epoch, and
                                     // re-execute transactions in the queue
      break;
    }
#else
    case 0: {
      // 1. pause database worker threads and abort all current transactions if
      // any
      control_mode = 1;
    ...
    case 2: { // receive a COMMIT
      ...
      // }
      // resume the database worker threads, update it after the new workers are
      // created;
...
// std::cout << "we are using jemalloc? " << (mallctl != nullptr) << std::endl;
  ...
  if (benchConfig.getConfig() && benchConfig.getConfig()->multi_shard_mode) {
    ...
    if (shard_ctx && shard_ctx->runtime.get()) {
      // Use get() and const_cast because get_mut() returns null with shared
      // ownership
      const_cast<SiloRuntime *>(shard_ctx->runtime.get())
          ->BindToCurrentThread();
    }
  } else {
  ...
  CpuThrottler throttler(benchConfig.getCpuLimitPercent(),
                         benchConfig.getThrottleCycleMs());
  if (throttler.is_enabled()) {
    std::cout << "Worker " << worker_id
              << " CPU throttling enabled: " << throttler.get_cpu_percent()
              << "% with " << throttler.get_cycle_ms() << "ms cycle"
              << std::endl;
  }

  while (benchConfig.isRunning() &&
         (benchConfig.getRunMode() != RUNMODE_OPS ||
          ntxn_commits < benchConfig.getOpsPerWorker())) {
    ...
    for (size_t i = 0; i < workload.size(); i++) {
      ...
      retry:
        ...
        if (likely(ret.first)) {
          ...
          ++ntxn_aborts;
          if (false && benchConfig.getRetryAbortedTransaction() &&
              benchConfig.isRunning()) { // don't retry
        ...
        if (ret.second % 10 == 1) {    // remote
          ...
          if (ret.first) {   // commit
            txn_counts[i]++; // txn_counts aren't used to compute throughput (is
                             // just an informative number to print to the
                             // console in verbose mode)
            txn_counts[i + 10] += tl;
          } else {               // abort
            txn_counts[i + 5]++; // txn_counts aren't used to compute throughput
                                 // (is just an informative number to print to
                                 // the console in verbose mode)
            txn_counts[i + 15] += tl;
          }
        }
        break;
  ...
  // pclock((char*)("[CPU_TIME] Database worker thread CPU time lock, id: " +
  // std::to_string(TThread::id()) + ": ").c_str(), cid);
...
void bench_runner::stop() { // invoke inside run function; stop all ShardClient
                            // instances
...
void bench_runner::run() {
  ...
  if (f_mode == 0) { // f_mode==0 is normal to load data
    {
      ...
      {
        scoped_timer t("dataloading",
                       BenchmarkConfig::getInstance().getVerbose());
        size_t N = loaders.size();
        Warning("# of loaders size:%d", N);
        auto &benchConfig = BenchmarkConfig::getInstance();
        for (int batch = 0; batch < (N / benchConfig.getNthreads()) + 1;
             batch++) {
      ...
      const int64_t delta = int64_t(mem_info_before.first) -
                            int64_t(mem_info_after.first); // free mem
    ...
    if (!BenchmarkConfig::getInstance().getNoResetCounters()) {
      event_counter::reset_all_counters(); // XXX: for now - we really should
                                           // have a before/after loading
      PERF_EXPR(scopedperf::perfsum_base::resetall());
    }
    {
      const auto persisted_info = db->get_ntxn_persisted();
      if (get<0>(persisted_info) != 0 || get<1>(persisted_info) != 0 ||
  ...
  if (BenchmarkConfig::getInstance().getVerbose()) {
    for (map<string, abstract_ordered_index *>::iterator it =
             open_tables.begin();
  ...
  if (f_mode == 0) {
    std::cout << "--------------Finish loading data and wait for others "
                 "completing load phase ------------"
              << std::endl;
    auto &cfg = BenchmarkConfig::getInstance();
    mako::NFSSync::set_key(
        "load_phase_" + std::to_string(cfg.getShardIndex()), "DONE",
        cfg.getConfig()->shard(0, cfg.getClusterRole()).host.c_str(),
        cfg.getConfig()->mports[cfg.getClusterRole()]);

    // wait for all other shards to complete
    for (int i = 0; i < cfg.getConfig()->nshards; i++) {
      if (i != cfg.getShardIndex()) {
        mako::NFSSync::wait_for_key(
            "load_phase_" + std::to_string(i),
            cfg.getConfig()->shard(0, cfg.getClusterRole()).host.c_str(),
            cfg.getConfig()->mports[cfg.getClusterRole()]);
      }
    }
    ...
    if (cfg.getIsReplicated()) {
      ...
      for (int i = 0; i < BenchmarkConfig::getInstance().getNthreads(); i++) {
        add_log_to_nc(log.c_str(), log.size(),
                      i); // notify others start a advancer
        std::cout << "[ADVANCER-SEND] Sent ADVANCER_MARKER to partition " << i
                  << std::endl;
      }
    }
  }
  ...
  // This ensures all shards have completed thread_init() before any start
  // transactions
  ...
  if (benchConfig.getRunMode() == RUNMODE_TIME) {
    ...
    while (runtime_loop > 0) {
      if (benchConfig.getShardIndex() == 0 &&
          ...
          benchConfig.getCluster().compare("localhost") ==
              0) { // 5 seconds, kill it on leader{0}
        uint32_t aa = getEpochInms();
        // Panic("STOP current process! tt:%llu", aa);
      }
      if (runtime_loop % repeats == 0)
        Warning("runtime time left:%d ms, bool:%d", runtime_loop * interval,
                runtime_loop > 0);
      if (runtime_loop % repeats == 0)
        std::cout << std::flush;
      runtime_loop--;
  ...
  // if it is the learner ==> it's the new leader and so it should be terminated
  // faster than others the eRPC client has to be used in the same thread that
  // created it, so we can't use client_control
  if (benchConfig.getCluster().compare("learner") == 0) {
    sync_util::sync_logger::client_control2(3, benchConfig.getShardIndex());
  }
  if (benchConfig.getRunMode() == RUNMODE_TIME) {
    cerr << "[SHUTDOWN] Setting running=false to stop database worker threads"
         << endl;
  ...
  cerr << "[SHUTDOWN] Joining " << BenchmarkConfig::getInstance().getNthreads()
       << " worker threads" << endl;
  ...
  // Stop server transports AFTER workers exit to ensure they can finish
  // processing
  ...
  const unsigned long elapsed_nosync =
      t_nosync.lap() -
      1e6; // take 1 second off due to sleep(1) within bench_worker::run()
  ...
  for (size_t i = 0; i < BenchmarkConfig::getInstance().getNthreads(); i++) {
    ...
    cerr << " par_id: " << i
         << ", n_commits: " << workers[i]->get_ntxn_commits() << endl;
  ...
  if (sync_util::sync_logger::hist_timestamp_vec.find(0) !=
      sync_util::sync_logger::hist_timestamp_vec.end()) {
    auto w = sync_util::sync_logger::hist_timestamp_vec[0][0];
    // w -> timestamp * 10 + epoch
    cerr << "--- failed shard: " << w << endl;
    // time in ms; <tput; good tput>; we do this analysis only up to the failure
    // dection point
    unordered_map<uint64_t, std::pair<uint64_t, uint64_t>>
        merged_rollback_tracker;
    for (size_t i = 0; i < BenchmarkConfig::getInstance().getNthreads(); i++) {
      ...
      for (const auto &it : rt) {
        ...
        merged_rollback_tracker[it.first] = std::make_pair(
            std::get<0>(merged_rollback_tracker[it.first]) + it.second.size(),
            std::get<1>(merged_rollback_tracker[it.first]) + c);
      }
    }
    std::vector<std::pair<uint64_t, std::pair<uint64_t, uint64_t>>> vec(
        merged_rollback_tracker.begin(), merged_rollback_tracker.end());
    std::sort(vec.begin(), vec.end(),
              [](const auto &a, const auto &b) { return a.first < b.first; });

    for (const auto &entry : vec) {
      std::cout << "Key: " << entry.first << ", Value: (" << entry.second.first
                << ", " << entry.second.second << ")\n";
    }
  }
#endif
  ...
  const unsigned long elapsed =
      t.lap() - 1e6; // lap() must come after do_txn_finish(),
  ...
  const double avg_persist_latency_ms = get<2>(persisted_info) / 1000.0;

  map<string, size_t> agg_txn_counts = workers[0]->get_txn_counts();
  // cerr << "[breakdown] TPUT worker-0: " <<
  // format_list(agg_txn_counts.begin(), agg_txn_counts.end()) << endl << endl;
  ...
  for (size_t i = 1; i < workers.size(); i++) {
    ...
    // cerr << "[breakdown] TPUT worker-" << i << ": " <<
    // format_list(tmp.begin(), tmp.end()) << endl << endl;
  }
  ...
  if (BenchmarkConfig::getInstance().getVerbose()) {
    ...
    //   scoped_rcu_region guard;
    //   const size_t s = it->second->size();
    //   const ssize_t delta = ssize_t(s) -
    //   ssize_t(table_sizes_before[it->first]); cerr << "table " << it->first
    //   << " size " << it->second->size(); if (delta < 0)
    ...
    cerr << "memory delta rate: " << (delta_mb / elapsed_sec) << " MB/sec"
         << endl;
    cerr << "logical memory delta: " << size_delta_mb << " MB" << endl;
    cerr << "logical memory delta rate: " << (size_delta_mb / elapsed_sec)
         << " MB/sec" << endl;
    ...
    cerr << "avg_per_core_throughput: " << avg_per_core_throughput
         << " ops/sec/core" << endl;
    cerr << "agg_persist_throughput: " << agg_persist_throughput << " ops/sec"
         << endl;
    cerr << "avg_per_core_persist_throughput: "
         << avg_per_core_persist_throughput << " ops/sec/core" << endl;
    ...
    string txn_w1[] = {"NewOrder", "Payment", "Delivery", "OrderStatus",
                       "StockLevel"};
    ...
    for (int i = 0; i < sizeof(txn_w1) / sizeof(txn_w1[0]); i++) {
      if (agg_txn_counts.find(txn_w1[i] + "_Local") != agg_txn_counts.end()) {
        cerr << "  " << txn_w1[i] << "_local_commit_latency: "
             << agg_txn_counts[txn_w1[i] + "_Local_NANO"] /
                    (agg_txn_counts[txn_w1[i] + "_Local"] + 0.0) / 1000000.0
             << " ms" << endl;
        cerr << "  " << txn_w1[i] << "_local_abort_latency: "
             << agg_txn_counts[txn_w1[i] + "_Local_NANO_abort"] /
                    (agg_txn_counts[txn_w1[i] + "_Local_abort"] + 0.0) /
                    1000000.0
             << " ms" << endl;
        cerr << "  " << txn_w1[i] << "_local_abort_ratio: "
             << agg_txn_counts[txn_w1[i] + "_Local_abort"] /
                    (agg_txn_counts[txn_w1[i] + "_Local"] +
                     agg_txn_counts[txn_w1[i] + "_Local_abort"] + 0.0)
             << endl;
      }
    }
    ...
    for (int i = 0; i < sizeof(txn_ratio) / sizeof(txn_ratio[0]); i++) {
      if (agg_txn_counts.find(txn_ratio[i] + "_Local") !=
              agg_txn_counts.end() &&
          agg_txn_counts.find(txn_ratio[i] + "_Remote") !=
              agg_txn_counts.end()) {
        cerr << "  " << txn_ratio[i] << "_remote_ratio: "
             << 100 *
                    (agg_txn_counts[txn_ratio[i] + "_Remote"] +
                     agg_txn_counts[txn_ratio[i] + "_Remote_abort"]) /
                    (agg_txn_counts[txn_ratio[i] + "_Local"] +
                     agg_txn_counts[txn_ratio[i] + "_Local_abort"] +
                     agg_txn_counts[txn_ratio[i] + "_Remote"] +
                     agg_txn_counts[txn_ratio[i] + "_Remote_abort"] + 0.0)
             << " %" << endl;
        cerr << "  " << txn_ratio[i] << "_remote_abort_ratio: "
             << 100 * agg_txn_counts[txn_ratio[i] + "_Remote_abort"] /
                    (agg_txn_counts[txn_ratio[i] + "_Remote_abort"] +
                     agg_txn_counts[txn_ratio[i] + "_Remote"] + 0.0)
             << " %" << endl;
        cerr << "  " << txn_w1[i] << "_remote_commit_latency: "
             << agg_txn_counts[txn_w1[i] + "_Remote_NANO"] /
                    (agg_txn_counts[txn_w1[i] + "_Remote"] + 0.0) / 1000000.0
             << " ms" << endl;
        cerr << "  " << txn_w1[i] << "_remote_abort_latency: "
             << agg_txn_counts[txn_w1[i] + "_Remote_NANO_abort"] /
                    (agg_txn_counts[txn_w1[i] + "_Remote_abort"] + 0.0) /
                    1000000.0
             << " ms" << endl;
      }
    }
  ...
  for (int i = 0; i < samplingTPUT.size(); i++) {
    cerr << "Time: " << samplingTPUT[i].first
         << ", n_comits: " << samplingTPUT[i].second << endl;
  }
...
template <typename K, typename V> struct map_maxer {
...
map<string, size_t> bench_worker::get_txn_counts() const {
  map<string, size_t> m;
  const workload_desc_vec workload = get_workload();
  for (size_t i = 0; i < workload.size(); i++) {
...
void bench_worker::print_stats() const {
  for (int i = 0; i < sampling_remote_calls.size(); i++)
    std::cout << "[work_id:" << worker_id << "]:" << sampling_remote_calls[i]
              << std::endl;
}
