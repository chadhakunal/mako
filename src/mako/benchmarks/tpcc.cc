 ...
 *
 * Specially,
 *   there is only one table in the micro-benchmark: items shared by all worker
 * threads
 */
...
#define TPCC_TABLE_LIST(x)                                                     \
  x(customer) x(customer_name_idx) x(district) x(history) x(new_order)         \
...
static inline ALWAYS_INLINE size_t NumWarehouses() {
  return (size_t)BenchmarkConfig::getInstance()
      .getScaleFactor(); // scale_factor ==
                         // BenchmarkConfig::getInstance().getNthreads()
}

static inline ALWAYS_INLINE size_t NumWarehousesTotal() {
  auto &cfg = BenchmarkConfig::getInstance();
  return cfg.getNshards() *
         ((size_t)cfg
              .getScaleFactor()); // scale_factor ==
                                  // BenchmarkConfig::getInstance().getNthreads()
}

static inline ALWAYS_INLINE size_t ShardIndexFromGlobalWarehouse(int g_wid) {
  return (g_wid - 1) / NumWarehouses();
}

static inline ALWAYS_INLINE size_t WarehouseLocal2Global(int l_wid) {
  return BenchmarkConfig::getInstance().getShardIndex() * NumWarehouses() +
         l_wid;
}

static inline ALWAYS_INLINE size_t WarehouseGlobal2Local(int g_wid) {
  return (g_wid - 1) % NumWarehouses() + 1;
}

static inline ALWAYS_INLINE void ALWAYS_ERROR(bool aa) {
  if (likely(aa)) {
  } else {
...
static inline ALWAYS_INLINE bool WarehouseInShard(int g_w_id, int sIdx) {
  return g_w_id >= sIdx * NumWarehouses() + 1 &&
         g_w_id <= sIdx * NumWarehouses() + NumWarehouses();
}
// #endif
...
template <typename T> class scoped_multilock {
public:
  inline scoped_multilock() : did_lock(false) {}
...
template <typename T> class scoped_lock_guard {
public:
  inline scoped_lock_guard(T &l) : l(&l) { this->l->lock(); }

  inline scoped_lock_guard(T *l) : l(l) {
    if (this->l)
      this->l->lock();
  }
...
#else
static unsigned g_txn_workload_mix[] = {45, 43, 4, 4,
                                        4}; // default TPC-C workload mix
#endif
...
static int nkeys =
    100 *
    10000; // 1 million, TODO: it's better for in proportion to scale_factor
...
static inline ALWAYS_INLINE spinlock &LockForPartition(unsigned int wid) {
...
struct checker {
  ...
  SanityCheckCustomer(const customer::key *k, const customer::value *v) {
    INVARIANT(k->c_w_id >= 1 &&
              static_cast<size_t>(k->c_w_id) <= NumWarehouses());
    INVARIANT(k->c_d_id >= 1 &&
  ...
  SanityCheckDistrict(const district::key *k, const district::value *v) {
    INVARIANT(k->d_w_id >= 1 &&
              static_cast<size_t>(k->d_w_id) <= NumWarehouses());
    INVARIANT(k->d_id >= 1 &&
  ...
  static inline ALWAYS_INLINE void SanityCheckStock(const stock::key *k,
                                                    const stock::value *v) {
    INVARIANT(k->s_w_id >= 1 &&
              static_cast<size_t>(k->s_w_id) <= NumWarehouses());
  ...
  SanityCheckNewOrder(const new_order::key *k, const new_order::value *v) {
    INVARIANT(k->no_w_id >= 1 &&
              static_cast<size_t>(k->no_w_id) <= NumWarehouses());
    INVARIANT(k->no_d_id >= 1 &&
  ...
  static inline ALWAYS_INLINE void SanityCheckOOrder(const oorder::key *k,
    ...
    INVARIANT(k->o_w_id >= 1 &&
              static_cast<size_t>(k->o_w_id) <= NumWarehouses());
    INVARIANT(k->o_d_id >= 1 &&
    ...
    INVARIANT(v->o_carrier_id >= 0 && static_cast<size_t>(v->o_carrier_id) <=
  ...
  SanityCheckOrderLine(const order_line::key *k, const order_line::value *v) {
    INVARIANT(k->ol_w_id >= 1 &&
              static_cast<size_t>(k->ol_w_id) <= NumWarehouses());
    INVARIANT(k->ol_d_id >= 1 &&
...
private:                                                                       \
  ...
  vector<abstract_ordered_index *> remote_tbl_##name##_remote_vec;             \
                                                                               \
protected:                                                                     \
  inline ALWAYS_INLINE abstract_ordered_index *tbl_##name(unsigned int wid) {  \
    INVARIANT(wid >= 1 && wid <= NumWarehouses());                             \
    INVARIANT(tbl_##name##_vec.size() == NumWarehouses());                     \
    return tbl_##name##_vec[wid - 1];                                          \
  }                                                                            \
  inline ALWAYS_INLINE abstract_ordered_index *remote_tbl_##name(              \
      unsigned int wid) {                                                      \
  ...
  static void PinToWarehouseId(unsigned int wid) {
    ...
    if (BenchmarkConfig::getInstance().getVerbose())
      cerr << "PinToWarehouseId(): coreid=" << coreid::core_id()
           << " pinned to whse=" << wid << " (partid=" << partid << ")" << endl;
...
public:
  ...
  static inline ALWAYS_INLINE size_t GetCustomerLastName(char *buf,
                                                         fast_random &r,
                                                         int num) {
...
public:
  generic_scan_callback() { values.reserve(10); }
  ...
  inline void print_warehouse_info() {
    std::cout << "# of records in the warehouse table: " << values.size()
              << std::endl;
    for (int i = 0; i < values.size(); i++) {
      std::cout << "  " << "id: " << ((warehouse::key)(values[i].first)).w_id
                << std::endl;
    }
    std::cout << std::endl;
  }
  ...
  inline int64_t print_customer_info() {
    std::cout << "# of records in the customer table: " << values.size()
              << std::endl;
    ...
    for (int i = 0; i < values.size(); i++) {
      std::cout << "  " << "key: " << ((customer::key)(values[i].first))
                << ", balance: "
                << ((customer::value)(values[i].second)).c_payment_cnt
                << std::endl;
      total_balance += ((customer::value)(values[i].second)).c_payment_cnt;
      total_remote_udpate +=
          ((customer::value)(values[i].second)).c_delivery_cnt;
    }
  ...
  inline int64_t print_oorder_info() {
    std::cout << "# of records in the oorder table: " << values.size()
              << std::endl;
    ...
    for (int i = 0; i < values.size(); i++) {
      std::cout << "  " << "key: " << ((oorder::key)(values[i].first))
                ...
                << ", customer-id: "
                << ((oorder::value)(values[i].second)).o_carrier_id
...
public:
  ...
  tpcc_worker(
      unsigned int worker_id, unsigned long seed, abstract_db *db,
      const map<string, abstract_ordered_index *> &open_tables,
      const map<string, vector<abstract_ordered_index *>> &partitions,
      const map<string, vector<abstract_ordered_index *>> &remote_partitions,
      spin_barrier *barrier_a, spin_barrier *barrier_b, uint warehouse_id_start,
      uint warehouse_id_end, int shard_index = -1)
      : bench_worker(worker_id, true, seed, db, open_tables, barrier_a,
                     barrier_b, shard_index),
    ...
    cerr << "tpcc: worker id " << worker_id << " => warehouses ["
         << warehouse_id_start << ", " << warehouse_id_end << ")" << endl;
  ...
  // ~tpcc_worker() {
  //   std::cout<<"worker "<<worker_id<<" new order failed
  //   "<<counter_new_order_failed<<std::endl; std::cout<<"worker
  //   "<<worker_id<<" payment failed "<<counter_payment_failed<<std::endl;
  // }
  ...
  static txn_result TxnNewOrder(bench_worker *w) {
    // if (BenchmarkConfig::getInstance().getShardIndex() == 1) { return
    // txn_result(true, 0); }
  ...
  static txn_result TxnPayment(bench_worker *w) {
    // if (BenchmarkConfig::getInstance().getShardIndex() == 1) { return
    // txn_result(true, 0); }
  ...
  static txn_result TxnOrderStatus(bench_worker *w) {
  ...
  static txn_result TxnStockLevel(bench_worker *w) {
  ...
  virtual workload_desc_vec get_workload() const {
    ...
    if (g_txn_workload_mix[2])
      w.push_back(workload_desc(
          "Delivery", double(g_txn_workload_mix[2]) / 100.0, TxnDelivery));
    if (g_txn_workload_mix[3])
      w.push_back(workload_desc("OrderStatus",
                                double(g_txn_workload_mix[3]) / 100.0,
                                TxnOrderStatus));
    if (g_txn_workload_mix[4])
      w.push_back(workload_desc(
          "StockLevel", double(g_txn_workload_mix[4]) / 100.0, TxnStockLevel));
#endif
    return w;
  }
...
protected:
  virtual void on_run_setup() OVERRIDE {
    ...
    // if
    // (BenchmarkConfig::getInstance().getClusterRole()==mako::LOCALHOST_CENTER_INT){
    // // disable it if 10 shards
    //  #if !defined(MEGA_BENCHMARK)
    //  for (int i=0;i<=100;i++) {
    //     ///*printf("start - pid:%d-i:%d", TThread::getGlobalPartitionID(),
    //     i);
    //     //std::cout<<std::endl;
    ...
    //    uint32_t ret_value;
    //    uint32_t req_val = i;
    //    TThread::sclient->warmupRequest(req_val, mako::LEARNER_CENTER_INT,
    //    ret_value, dstShardIndex); int ret = ret_value;
    //  }
    //  Warning("DONE a warmup on leader:%d to the learner-0\n",
    //  TThread::getGlobalPartitionID()); #endif
    //}
  }
...
protected:
  void load_simple() {
    // load warehouses
    string obj_buf;
    void *txn = db->new_txn(BenchmarkConfig::getInstance().getTxnFlags(), arena,
                            txn_buf());
    try {
      ...
      for (uint w_id = 1; w_id <= NumWarehouses(); w_id++)
        for (uint c_id = 1; c_id <= 10; c_id++) {
          arena.reset();
          void *txn_1 = db->new_txn(
              BenchmarkConfig::getInstance().getTxnFlags(), arena, txn_buf());
  ...
  void load_micro() {
    ...
    for (size_t batchid = 0; batchid < nbatches;) {
      ...
      void *const txn = db->new_txn(
          BenchmarkConfig::getInstance().getTxnFlags(), arena, txn_buf());
      try {
        const size_t rend = (batchid + 1 == nbatches)
                                ? keyend
                                : keystart + ((batchid + 1) * batchsize);
        ...
        Notice("[load] micro-benchmark insert: [%d,%d]",
               batchid * batchsize + keystart, rend);
  ...
  virtual void load() {
    ...
    try {
      ...
      ALWAYS_ERROR(db->commit_txn(txn));
      arena.reset();
      txn = db->new_txn(BenchmarkConfig::getInstance().getTxnFlags(), arena,
                        txn_buf());
      for (uint i = 1; i <= NumWarehouses(); i++) {
        const warehouse::key k(i);
        string warehouse_v;
        ...
        if (TThread::transget_without_stable) {
  ...
  virtual void load() {
    ...
    try {
      for (uint i = 1; i <= NumItems(); i++) {
        ...
        tbl_item(1)->insert(
            txn, EncodeK(k),
            Encode(obj_buf, v)); // this table is shared, so any partition is OK
...
public:
  tpcc_stock_loader(
      ...
      const map<string, vector<abstract_ordered_index *>> &partitions,
      const map<string, vector<abstract_ordered_index *>> &remote_partitions,
      ssize_t warehouse_id, int shard_index = -1)
      : bench_loader(seed, db, open_tables, shard_index),
        tpcc_worker_mixin(partitions, remote_partitions),
  ...
  virtual void load() {
    ...
    for (uint w = w_start; w <= w_end; w++) {
      ...
      const size_t nbatches =
          (batchsize > NumItems()) ? 1 : (NumItems() / batchsize);
      ...
      for (uint b = 0; b < nbatches;) {
        ...
        try {
          ...
          // Warning("insert stock: w:%d, range:(%d-%d)",w,(b * batchsize +
          // 1),iend);
          for (uint i = (b * batchsize + 1); i <= iend; i++) {
            ...
            if (RandomNumber(r, 1, 100) > 10) {
              ...
              const int startOriginal = RandomNumber(r, 2, (len - 8));
              const string s_data = RandomStr(r, startOriginal + 1) +
                                    "ORIGINAL" +
                                    RandomStr(r, len - startOriginal - 7);
              v_data.s_data.assign(s_data);
            }
            ...
            tbl_stock_data(w)->insert(txn, EncodeK(k_data),
                                      Encode(obj_buf1, v_data));
          }
          if (db->commit_txn(txn)) {
            b++;
          } else {
      ...
      for (uint w = 1; w <= NumWarehouses(); w++) {
        ...
        for (uint d = 1; d <= NumDistrictsPerWarehouse(); d++, cnt++) {
          ...
          if (bsize != -1 && !((cnt + 1) % bsize)) {
            ALWAYS_ERROR(db->commit_txn(txn));
            txn = db->new_txn(BenchmarkConfig::getInstance().getTxnFlags(),
                              arena, txn_buf());
            arena.reset();
          }
        }
        ...
        for (uint batch = 0; batch < nbatches;) {
          ...
          try {
            for (uint cidx0 = cstart; cidx0 < cend; cidx0++) {
              ...
              tbl_customer(w)->insert(txn, EncodeK(k_balance),
                                      Encode(obj_buf, v_balance));
              ...
              tbl_customer(w)->insert(txn, EncodeK(k_data),
                                      Encode(obj_buf, v_data));
              ...
              tbl_customer_name_idx(w)->insert(txn, Encode(k_idx),
                                               Encode(obj_buf, v_idx));
              ...
              tbl_history(w)->insert(txn, EncodeK(k_hist),
                                     Encode(obj_buf, v_hist));
            }
            if (db->commit_txn(txn)) {
              batch++;
            } else {
      ...
      for (uint d = 1; d <= NumDistrictsPerWarehouse(); d++) {
        ...
        for (uint c = 1; c <= NumCustomersPerDistrict();) {
          ...
          try {
            ...
            tbl_oorder_c_id_idx(w)->insert(txn, Encode(k_oo_idx),
                                           Encode(obj_buf, v_oo_idx));
            ...
            if (c >= 2101) {
              ...
              n_new_orders++;
              tbl_new_order(w)->insert(txn, Encode(k_no),
                                       Encode(obj_buf, v_no));
            }
            ...
            for (uint l = 1; l <= uint(v_oo.o_ol_cnt); l++) {
              ...
              n_order_lines++;
              tbl_order_line(w)->insert(txn, Encode(k_ol),
                                        Encode(obj_buf, v_ol));
            }
            if (db->commit_txn(txn)) {
              c++;
...
static event_counter evt_tpcc_cross_partition_new_order_txns(
    "tpcc_cross_partition_new_order_txns");
static event_counter
    evt_tpcc_cross_partition_payment_txns("tpcc_cross_partition_payment_txns");
...
void tpcc_worker::scan_entire_warehouses(int w_id) {
  ...
  // arena.reset();
  // void *txn_0 = db->new_txn(BenchmarkConfig::getInstance().getTxnFlags(),
  // arena, txn_buf(), abstract_db::HINT_DEFAULT);
  ...
  arena.reset();
  void *txn_1 = db->new_txn(BenchmarkConfig::getInstance().getTxnFlags(), arena,
                            txn_buf(), abstract_db::HINT_DEFAULT);
...
// workload:
// https://docs.google.com/document/d/11DqnIV0aC5aSV-uVg8SVST_5Ij4BCutfOQJdH7Fdr1s/edit?usp=sharing
tpcc_worker::txn_result tpcc_worker::txn_new_order_simple() {
  bool isRemote = false;
  bool isAbort = false;
  ...
  if (NumWarehousesTotal() > 1 && TThread::get_nshards() > 1)
    do {
      ...
      // } while (WarehouseInShard(remote_warehouse_id,
  ...
  std::vector<int> ids;
  {
    ...
    retry:
      arena.reset();
      void *txn = db->new_txn(BenchmarkConfig::getInstance().getTxnFlags(),
                              arena, txn_buf(), abstract_db::HINT_DEFAULT);
      try {
        // Tranasction-1
        {
          ...
          ALWAYS_ERROR(tbl_customer(warehouse_id)
                           ->get(txn, EncodeK(obj_key0, k_c), obj_v));
          if (TThread::transget_without_stable) {
            TThread::transget_without_stable = false;
          }
          if (TThread::transget_without_throw) {
            TThread::transget_without_throw = false;
            db->abort_txn_local(txn);
            return txn_result(false, is_remote ? 1 : 0);
          }
          ...
          tbl_customer(warehouse_id)
              ->put(txn, Encode(str(), k_c), Encode(str(), v_c_new));
          const oorder::key k_oo(warehouse_id, 1, oorder_id);
          oorder::value v_oo_new;
          v_oo_new.o_c_id = 10;
          v_oo_new.o_carrier_id = c_id;
          tbl_oorder(warehouse_id)
              ->insert(txn, Encode(k_oo), Encode(str(), v_oo_new));

          if (remote_warehouse_id > 0 &&
              !WarehouseInShard(
                  remote_warehouse_id,
                  BenchmarkConfig::getInstance().getShardIndex()) &&
              (c_id == 4 || c_id == 5)) {
            customer::key r_k_c(WarehouseGlobal2Local(remote_warehouse_id), 1,
                                c_id);
            // Warning("[DEBUG]client tries to get,
            // w_id:%d,d:%d,c_id:%d,oorder_id:%d,len of
            // obj_v(garbage):%d",WarehouseGlobal2Local(remote_warehouse_id), 1,
            // c_id,oorder_id,obj_v.size());
            ALWAYS_ERROR(remote_tbl_customer(remote_warehouse_id)
                             ->get(txn, Encode(obj_key0, r_k_c), obj_v));
            customer::value r_v_c_temp;
            // Warning("# of
            // obj_v:%d,r_v_c_temp:%d,(w:%d,d:%d,o:%d)",obj_v.size(),Size(r_v_c_temp),WarehouseGlobal2Local(remote_warehouse_id),
            // 1, c_id);
            ...
            remote_tbl_customer(remote_warehouse_id)
                ->put(txn, EncodeK(str(), r_k_c), Encode(str(), r_v_c_new));
            is_remote = true;
          }
        }
        ...
        if (!ret) {
          Warning(
              "fail to transaction-1(false):%d,local_oorder_id:%d,par_id:%d",
              warehouse_id, local_oorder_id, TThread::getGlobalPartitionID());
          isAbort = true;
          // db->abort_txn(txn);
          // goto retry;
        } else {
          // Warning("succeed to
          // transaction-1:%d,local_oorder_id:%d,par_id:%d,c_id:%d,is_remote:%d",
          // warehouse_id,local_oorder_id,
          // TThread::getGlobalPartitionID(),c_id,is_remote);
        }
      } catch (abstract_db::abstract_abort_exception &ex) {
        Warning("fail to transaction-1(abort):%d,local_oorder_id:%d,par_id:%d",
                warehouse_id, local_oorder_id, TThread::getGlobalPartitionID());
        db->abort_txn(txn);
        isAbort = true;
      } catch (int n) {
      ...
      // Transaction-2
      {
        ...
        try {
          ...
          if (!ret) {
            Warning(
                "fail to transaction-2(false):%d,local_oorder_id:%d,par_id:%d",
                warehouse_id, local_oorder_id, TThread::getGlobalPartitionID());
            // db->abort_txn(txn);
            isAbort = true;
          } else {
            Warning("succeed to transaction-2:%d,local_oorder_id:%d,par_id:%d",
                    warehouse_id, local_oorder_id,
                    TThread::getGlobalPartitionID());
          }
        } catch (abstract_db::abstract_abort_exception &ex) {
          Warning(
              "fail to transaction-2(abort):%d,local_oorder_id:%d,par_id:%d",
              warehouse_id, local_oorder_id, TThread::getGlobalPartitionID());
          db->abort_txn(txn);
          isAbort = true;
        } catch (int n) {
  ...
  if (isAbort || local_oorder_id % 10 == 0) {
    Warning("current oorder id: %d, w-id: %d,par_id:%d", local_oorder_id,
            warehouse_id, TThread::getGlobalPartitionID());
    try {
      usleep((rand() % 100) * 1000);
  ...
  for (int i=0; i<ids.size(); i++) {
    arena.reset();
    void *txn = db->new_txn(BenchmarkConfig::getInstance().getTxnFlags(), arena,
  txn_buf(), abstract_db::HINT_DEFAULT); const oorder::key k_oo(warehouse_id, 1,
  ids.at(i)); bool tag = tbl_oorder(warehouse_id)->get(txn, EncodeK(obj_key0,
  k_oo), obj_v); std::vector<string> allVersion =
  MultiVersionValue::getAllVersion<oorder::value>(obj_v); std::cout <<
  std::endl; int counter=0; std::cout << "key: " << k_oo << std::endl; std::cout
  << "value: " << std::endl; for (int i=0;i<allVersion.size();i++) { counter ++;
...
txn_buf(), abstract_db::HINT_DEFAULT);
    {
      ...
      oorder::value v_oo_new;
      v_oo_new.o_c_id = 1;
      tbl_oorder(warehouse_id)->insert(txn, Encode(k_oo), Encode(str(),
v_oo_new)); bool ret = db->commit_txn(txn); if (!ret){ Warning("fail to insert
oorder_id:%d",oorder_id);
      }
    }
  }
  ...
  for (int oorder_id=0; oorder_id<n; oorder_id++) {
    ...
    void *txn = db->new_txn(BenchmarkConfig::getInstance().getTxnFlags(), arena,
txn_buf(), abstract_db::HINT_DEFAULT); const oorder::key k_oo(warehouse_id, 1,
oorder_id); tbl_oorder(warehouse_id)->remove(txn, Encode(str(), k_oo)); bool ret
= db->commit_txn(txn); if (!ret){ Warning("fail to remove
oorder_id:%d",oorder_id);
    }
  }
...
txn_buf(), abstract_db::HINT_DEFAULT);
    {
      const oorder::key k_oo(warehouse_id, 1, oorder_id);
      bool tag = tbl_oorder(warehouse_id)->get(txn, EncodeK(obj_key0, k_oo),
obj_v); std::vector<string> allVersion =
MultiVersionValue::getAllVersion<oorder::value>(obj_v); std::cout << std::endl;
      int counter=0;
  ...
  // scan the range: [3,7)
  {
    arena.reset();
    void *txn = db->new_txn(BenchmarkConfig::getInstance().getTxnFlags(), arena,
txn_buf(), abstract_db::HINT_DEFAULT); scoped_str_arena s_arena(arena); const
oorder::key k_no_0(warehouse_id, 1, 3); const oorder::key k_no_1(warehouse_id,
1, 7); generic_scan_callback<oorder::key, oorder::value> calloc;
    tbl_oorder(warehouse_id)->scan(txn, Encode(obj_key0, k_no_0),
&Encode(obj_key1, k_no_1), calloc, s_arena.get());
    ALWAYS_ERROR(db->commit_txn(txn));
...
tpcc_worker::txn_result tpcc_worker::txn_new_order_micro_drtm() {
  bool isRemote = false;
  uint warehouse_id = PickWarehouseId(r, warehouse_id_start, warehouse_id_end);
  ...
  // TODO: the event driven might is the bottleneck for the microbenchmark (much
  // high throughput)
  void *txn = db->new_txn(BenchmarkConfig::getInstance().getTxnFlags(), arena,
                          txn_buf(), abstract_db::HINT_KV_RMW);
  scoped_str_arena s_arena(arena);
  try {
    ...
    for (int i = 0; i < RMW_count; i++) {
      ...
      if (isRemote && !first) {
        first = true;
        ALWAYS_ERROR(
            remote_tbl_item(remote_warehouse_id)->get(txn, EncodeK(k), obj_v));
        { // optimistic replication
          ...
          remote_tbl_item(remote_warehouse_id)
              ->put(txn, Encode(str(), k), Encode(str(), v_c_new));
...
tpcc_worker::txn_result tpcc_worker::txn_new_order_micro() {
  bool isRemote = false;
  uint warehouse_id = PickWarehouseId(r, warehouse_id_start, warehouse_id_end);
    ...
    for (int i = 0; i < RMW_count; i++) {
      ...
      if (isRemote && !first) {
        ...
        ALWAYS_ERROR(
            remote_tbl_item(remote_warehouse_id)->get(txn, EncodeK(k), obj_v));
        remote_tbl_item(remote_warehouse_id)
            ->put(txn, Encode(str(), k), Encode(str(), v));
      } else {
        ALWAYS_ERROR(tbl_item(1)->get(txn, EncodeK(k), obj_v));
        if (TThread::transget_without_stable) {
          TThread::transget_without_stable = false;
...
tpcc_worker::txn_result tpcc_worker::txn_new_order_micro_mega() {
  bool isRemote = false;
  uint warehouse_id = PickWarehouseId(r, warehouse_id_start, warehouse_id_end);
...
tpcc_worker::txn_result tpcc_worker::txn_new_order_mega() {
  const uint warehouse_id =
      PickWarehouseId(r, warehouse_id_start, warehouse_id_end);
  ...
  bool allLocal = true;  // not on the local warehouse, but it's possible still
                         // on the same shard server
  bool isRemote = false; // on the remote shard server
  int reallyRemote = 0;
  for (uint i = 0; i < numItems; i++) {
    itemIDs[i] = GetItemId(r); // 100000
    if (likely(g_disable_xpartition_txn || NumWarehousesTotal() == 1 ||
               // i > 0 ||
      ...
      do {
        supplierWarehouseIDs[i] = RandomNumber(r, 1, NumWarehousesTotal());
               ...
               ((!WarehouseInShard(
                    supplierWarehouseIDs[i],
                    BenchmarkConfig::getInstance().getShardIndex())) &&
                reallyRemote >= 1));
      if (!WarehouseInShard(supplierWarehouseIDs[i],
                            BenchmarkConfig::getInstance().getShardIndex())) {
        reallyRemote++;
      }
      allLocal = false;
  ...
  //   num_txn_contexts : 9
  void *txn = db->new_txn(BenchmarkConfig::getInstance().getTxnFlags(), arena,
                          txn_buf(), abstract_db::HINT_TPCC_NEW_ORDER);
  scoped_str_arena s_arena(arena);
  ...
  if (g_enable_partition_locks) { // partition-locks is disable by default
    if (allLocal) {
      ...
      for (uint i = 0; i < numItems; i++) {
        if (lockset.find(PartitionId(supplierWarehouseIDs[i])) ==
            lockset.end()) {
  ...
  try {
    ...
    for (int i = 0; i < batch_size; i++) {
      const customer::key k_c(warehouse_id, districtID,
                              customerID + i > 3000 ? customerID + i - 3000
                                                    : customerID + i);
      ALWAYS_ERROR(
          tbl_customer(warehouse_id)->get(txn, EncodeK(obj_key0, k_c), obj_v));
      if (TThread::transget_without_stable) {
        TThread::transget_without_stable = false;
      }
      if (TThread::transget_without_throw) {
    ...
    ALWAYS_ERROR(
        tbl_warehouse(warehouse_id)->get(txn, Encode(obj_key0, k_w), obj_v));
    if (TThread::transget_without_stable) {
      TThread::transget_without_stable = false;
    }
    if (TThread::transget_without_throw) {
    ...
    ALWAYS_ERROR(
        tbl_district(warehouse_id)->get(txn, Encode(obj_key0, k_d), obj_v));
    if (TThread::transget_without_stable) {
      TThread::transget_without_stable = false;
    }
    if (TThread::transget_without_throw) {
    ...
    for (int i = 0; i < batch_size; i++) {
      ...
      tbl_new_order(warehouse_id)
          ->insert(txn, Encode(str(), k_no), Encode(str(), v_no));
      ret += new_order_sz;
      ...
      tbl_oorder(warehouse_id)
          ->insert(txn, EncodeK(str(), k_oo), Encode(str(), v_oo));
      ret += oorder_sz;
      ...
      tbl_oorder_c_id_idx(warehouse_id)
          ->insert(txn, Encode(str(), k_oo_idx), Encode(str(), v_oo_idx));
    }
    ...
    for (uint ol_number = 1; ol_number <= numItems; ol_number++) {
      ...
      for (int i = 0; i < batch_size; i++) {
        ol_i_id = base_ol_i_id + i > 100000 ? base_ol_i_id + i - 100000
                                            : base_ol_i_id + i;
        ...
        // local operations, for remote operations, we do it outside this
        // for-loop
        if (WarehouseInShard(ol_supply_w_id,
                             BenchmarkConfig::getInstance().getShardIndex())) {
          const stock::key k_s(WarehouseGlobal2Local(ol_supply_w_id), ol_i_id);
          ALWAYS_ERROR(tbl_stock(WarehouseGlobal2Local(ol_supply_w_id))
                           ->get(txn, EncodeK(obj_key0, k_s), obj_v));
          if (TThread::transget_without_stable) {
            TThread::transget_without_stable = false;
          }
          if (TThread::transget_without_throw) {
          ...
          v_s_new.s_remote_cnt +=
              (ol_supply_w_id == WarehouseLocal2Global(warehouse_id)) ? 0 : 1;

          tbl_stock(WarehouseGlobal2Local(ol_supply_w_id))
              ->put(txn, EncodeK(str(), k_s), Encode(str(), v_s_new));
        }
        const order_line::key k_ol(warehouse_id, districtID, no_o_ids[i],
                                   ol_number);
        order_line::value v_ol;
        ...
        tbl_order_line(warehouse_id)
            ->insert(txn, Encode(str(), k_ol), Encode(str(), v_ol));
        ret += order_line_sz;
      }
      ...
      if (!WarehouseInShard(ol_supply_w_id,
                            BenchmarkConfig::getInstance().getShardIndex())) {
        const stock::key k_s(WarehouseGlobal2Local(ol_supply_w_id),
                             base_ol_i_id);
        remote_tbl_stock(ol_supply_w_id)
            ->get(txn, EncodeK(obj_key0, k_s), obj_v);
        // std::cout<<"send base:"<<base_ol_i_id<<",
        // tid:"<<TThread::getGlobalPartitionID()<<", v-len:"<<obj_v.length()<<
        // std::endl;

        // batch-read
        for (int i = 0; i < mako::mega_batch_size; i++) {
          // cast away const
          // stock::value *v_s= const_cast<stock::value*>(reinterpret_cast<const
          // stock::value*>(obj_v.data()+i*mako::size_per_stock_value));
          stock::value *v_s = const_cast<stock::value *>(
              reinterpret_cast<const stock::value *>(obj_v.data()));
    ...
    if (likely(db->commit_txn(txn))) {
      // if (BenchmarkConfig::getInstance().getControlMode()==2 &&
      // counter_new_order_failed>0 &&
      // BenchmarkConfig::getInstance().getShardIndex()>0){
      //   Warning("counter_new_order_failed
...
tpcc_worker::txn_result tpcc_worker::txn_new_order() {
  if (TThread::get_is_micro()) {
    if (drtm)
  ...
  for (uint i = 0; i < numItems; i++) {
    ...
    if (likely(g_disable_xpartition_txn || NumWarehousesTotal() == 1 ||
      ...
      if (BenchmarkConfig::getInstance().getControlMode() ==
          4) { // distributed transaction without failed shard
        if (ShardIndexFromGlobalWarehouse(supplierWarehouseIDs[i]) ==
            sync_util::sync_logger::failed_shard_index) {
          counter_new_order_failed += 1;
          return txn_result(false, 0 + (isRemote ? 1 : 0));
        }
      }
#else
      if (BenchmarkConfig::getInstance().getControlMode() ==
          1) { // distributed transaction without failed shard
        if (ShardIndexFromGlobalWarehouse(supplierWarehouseIDs[i]) ==
            sync_util::sync_logger::failed_shard_index) {
          counter_new_order_failed += 1;
          return txn_result(false, 0 + (isRemote ? 1 : 0));
        }
      } else if (BenchmarkConfig::getInstance().getControlMode() ==
...
#if defined(FAIL_NEW_VERSION)
  if (BenchmarkConfig::getInstance().getControlMode() == 5 &&
      counter_new_order_failed > 0 &&
      BenchmarkConfig::getInstance().getShardIndex() > 0) {
    if (rand() % 20 == 0) {
      counter_new_order_failed -= 1;
      supplierWarehouseIDs[0] = RandomNumber(
          r, 1, NumWarehouses()); // always assume the shard-0 failed
      isRemote = true;
    }
  }
#else
  ...
  try {
    ...
    if (!g_new_order_fast_id_gen) {
      ...
      tbl_district(warehouse_id)
          ->put(txn, Encode(str(), k_d), Encode(str(), v_d_new));
    }
    ...
    for (uint ol_number = 1; ol_number <= numItems; ol_number++) {
      ...
      if (WarehouseInShard(ol_supply_w_id,
        ...
        bool ret = remote_tbl_stock(ol_supply_w_id)
                       ->get(txn, EncodeK(obj_key0, k_s), obj_v);
...
STATIC_COUNTER_DECL(scopedperf::tod_ctr, delivery_probe0_tod,
                    delivery_probe0_cg)

tpcc_worker::txn_result tpcc_worker::txn_delivery() {
  const uint warehouse_id =
      PickWarehouseId(r, warehouse_id_start, warehouse_id_end);
  ...
  //   num_txn_contexts : 4
  void *txn = db->new_txn(BenchmarkConfig::getInstance().getTxnFlags(), arena,
                          txn_buf(), abstract_db::HINT_TPCC_DELIVERY);
  scoped_str_arena s_arena(arena);
  ...
  try {
    ...
    for (uint d = 1; d <= NumDistrictsPerWarehouse(); d++) {
      ...
      new_order_scan_callback new_order_c;
      {
        ...
        tbl_new_order(warehouse_id)
            ->scan(txn, Encode(obj_key0, k_no_0), &Encode(obj_key1, k_no_1),
                   new_order_c, s_arena.get());
      }
      ...
      if (unlikely(!tbl_oorder(warehouse_id)
                        ->get(txn, EncodeK(obj_key0, k_oo), obj_v))) {
        if (TThread::transget_without_stable) {
          TThread::transget_without_stable = false;
        }
        if (TThread::transget_without_throw) {
      ...
      tbl_order_line(warehouse_id)
          ->scan(txn, Encode(obj_key0, k_oo_0), &Encode(obj_key1, k_oo_1), c,
                 s_arena.get());
      float sum = 0.0;
      for (size_t i = 0; i < c.size(); i++) {
        ...
        tbl_order_line(warehouse_id)
            ->put(txn, *c.values[i].first, Encode(str(), v_ol_new));
      }
      ...
      tbl_oorder(warehouse_id)
          ->put(txn, EncodeK(str(), k_oo), Encode(str(), v_oo_new));

      const uint c_id = v_oo->o_c_id;
      const float ol_total = sum;

      // const customer::key k_c(warehouse_id, d, c_id);
      // ALWAYS_ERROR(tbl_customer(warehouse_id)->get(txn, EncodeK(obj_key0,
      // k_c), obj_v));
      ...
      // tbl_customer(warehouse_id)->put(txn, EncodeK(str(), k_c), Encode(str(),
      // v_c_new));

      const customer::key k_c_new(warehouse_id, d + 200, c_id);
      ALWAYS_ASSERT(tbl_customer(warehouse_id)
                        ->get(txn, EncodeK(obj_key0, k_c_new), obj_v));
      if (TThread::transget_without_stable) {
        TThread::transget_without_stable = false;
      }
      if (TThread::transget_without_throw) {
      ...
      tbl_customer(warehouse_id)
          ->put(txn, EncodeK(str(), k_c_new), Encode(str(), v_c_b_new));
    }
...
static event_avg_counter
    evt_avg_cust_name_idx_scan_size("avg_cust_name_idx_scan_size");

tpcc_worker::txn_result tpcc_worker::txn_payment_micro_drtm() {
  bool isRemote = false;
  uint warehouse_id = PickWarehouseId(r, warehouse_id_start, warehouse_id_end);
...
tpcc_worker::txn_result tpcc_worker::txn_payment_micro_mega() {
  bool isRemote = false;
  uint warehouse_id = PickWarehouseId(r, warehouse_id_start, warehouse_id_end);
...
tpcc_worker::txn_result tpcc_worker::txn_payment_micro() {
  bool isRemote = false;
  uint warehouse_id = PickWarehouseId(r, warehouse_id_start, warehouse_id_end);
...
tpcc_worker::txn_result tpcc_worker::txn_payment() {
  if (TThread::get_is_micro()) {
    if (drtm)
...
#else
  if (BenchmarkConfig::getInstance().getControlMode() ==
      1) { // distributed transactions without failed shard
    if (ShardIndexFromGlobalWarehouse(customerWarehouseID) ==
        sync_util::sync_logger::failed_shard_index) {
      counter_payment_failed += 1;
      return txn_result(false, 0 + (isRemote ? 1 : 0));
...
#endif
  if (!WarehouseInShard(customerWarehouseID,
                        BenchmarkConfig::getInstance().getShardIndex())) {
    isRemote = true;
    TThread::isRemoteShard = true;
  }
  ...
  //   num_txn_contexts : 5
  void *txn = db->new_txn(BenchmarkConfig::getInstance().getTxnFlags(), arena,
                          txn_buf(), abstract_db::HINT_TPCC_PAYMENT);
  scoped_str_arena s_arena(arena);
  ...
  try {
    ...
    tbl_warehouse(warehouse_id)
        ->put(txn, Encode(str(), k_w), Encode(str(), v_w_new));
    ...
    if (RandomNumber(r, 1, 100) <= 60) {
      ...
      if (WarehouseInShard(customerWarehouseID,
                           BenchmarkConfig::getInstance().getShardIndex())) {
        tbl_customer_name_idx(WarehouseGlobal2Local(customerWarehouseID))
            ->scan(txn, Encode(obj_key0, k_c_idx_0),
                   &Encode(obj_key1, k_c_idx_1), c, s_arena.get());
        ...
        const customer_name_idx::value *v_c_idx =
            Decode(*c.values[index].second, v_c_idx_temp);
        k_c.c_id = v_c_idx->c_id;
      } else {
        remote_tbl_customer_name_idx(customerWarehouseID)
            ->scanRemoteOne(txn, Encode(obj_key0, k_c_idx_0),
                            Encode(obj_key1, k_c_idx_1), obj_v);
        customer_name_idx::value v_c_idx_temp;
      ...
      if (WarehouseInShard(customerWarehouseID,
                           BenchmarkConfig::getInstance().getShardIndex())) {
        ALWAYS_ERROR(tbl_customer(WarehouseGlobal2Local(customerWarehouseID))
                         ->get(txn, EncodeK(obj_key0, k_c), obj_v));
        if (TThread::transget_without_stable) {
          TThread::transget_without_stable = false;
          counter_payment_failed += 1;
        ...
        ALWAYS_ERROR(remote_tbl_customer(customerWarehouseID)
                         ->get(txn, EncodeK(obj_key0, k_c), obj_v));
      }
    ...
    if (WarehouseInShard(customerWarehouseID,
      ...
      tbl_customer(WarehouseGlobal2Local(customerWarehouseID))
          ->put(txn, EncodeK(str(), k_c), Encode(str(), v_c_new));
    } else {
      remote_tbl_customer(customerWarehouseID)
          ->put(txn, EncodeK(str(), k_c), Encode(str(), v_c_new));
    }
    ...
    int n =
        snprintf((char *)v_h.h_data.data(), v_h.h_data.max_size() + 1,
    ...
    tbl_history(warehouse_id)
        ->insert(txn, EncodeK(str(), k_h), Encode(str(), v_h));
    ret += history_sz;
    ...
    if (strncmp(v_c.c_credit.data(), "BC", 2) == 0) {
      ...
      int n = snprintf(buf, sizeof(buf), "%d %d %d %d %d %d | %s", k_c.c_id,
                       k_c.c_d_id, k_c.c_w_id, districtID, warehouse_id,
                       (int)paymentAmount, v_c_data.c_data.c_str());
      v_c_data_new.c_data.resize_junk(
          min(static_cast<size_t>(n), v_c_data_new.c_data.max_size()));
      NDB_MEMCPY((void *)v_c_data_new.c_data.data(), &buf[0],
                 v_c_data_new.c_data.size());

      if (WarehouseInShard(customerWarehouseID,
                           BenchmarkConfig::getInstance().getShardIndex())) {
        tbl_customer(WarehouseGlobal2Local(customerWarehouseID))
            ->put(txn, EncodeK(str(), k_c), Encode(str(), v_c_data_new));
      } else {
        remote_tbl_customer(customerWarehouseID)
            ->put(txn, EncodeK(str(), k_c), Encode(str(), v_c_data_new));
      }
...
STATIC_COUNTER_DECL(scopedperf::tod_ctr, order_status_probe0_tod,
                    order_status_probe0_cg)

tpcc_worker::txn_result tpcc_worker::txn_order_status() {
  const uint warehouse_id =
      PickWarehouseId(r, warehouse_id_start, warehouse_id_end);
  const uint districtID = RandomNumber(r, 1, NumDistrictsPerWarehouse());
  ...
  const abstract_db::TxnProfileHint hint =
      g_disable_read_only_scans ? abstract_db::HINT_TPCC_ORDER_STATUS
                                : abstract_db::HINT_TPCC_ORDER_STATUS_READ_ONLY;
  void *txn =
      db->new_txn(BenchmarkConfig::getInstance().getTxnFlags() | read_only_mask,
                  arena, txn_buf(), hint);
  scoped_str_arena s_arena(arena);
  ...
  try {
    ...
    if (RandomNumber(r, 1, 100) <= 60) {
      ...
      static_limit_callback<NMaxCustomerIdxScanElems> c(
          s_arena.get(), true); // probably a safe bet for now
      tbl_customer_name_idx(warehouse_id)
          ->scan(txn, Encode(obj_key0, k_c_idx_0), &Encode(obj_key1, k_c_idx_1),
                 c, s_arena.get());
      ALWAYS_ERROR(c.size() > 0);
      INVARIANT(c.size() < NMaxCustomerIdxScanElems); // we should detect this
      int index = c.size() / 2;
    ...
    if (g_order_status_scan_hack) {
      ...
      // read the first order by any customer.  To make this more interesting,
      // we randomly select which elem to pick within the 1st or 2nd btree
      // nodes. This is obviously a deviation from TPC-C, but it shouldn't make
      // that much of a difference in terms of performance numbers (in fact we
                                            ...
                                            numeric_limits<int32_t>::max());
      {
        ...
        tbl_oorder_c_id_idx(warehouse_id)
            ->scan(txn, Encode(obj_key0, k_oo_idx_0),
                   &Encode(obj_key1, k_oo_idx_1), c_oorder, s_arena.get());
      }
      ...
      const oorder_c_id_idx::key k_oo_idx_hi(warehouse_id, districtID, k_c.c_id,
                                             numeric_limits<int32_t>::max());
      tbl_oorder_c_id_idx(warehouse_id)
          ->rscan(txn, Encode(obj_key0, k_oo_idx_hi), nullptr, c_oorder,
                  s_arena.get());
      ALWAYS_ERROR(c_oorder.size() == 1);
    }

    oorder_c_id_idx::key k_oo_idx_temp;
    const oorder_c_id_idx::key *k_oo_idx =
        Decode(*newest_o_c_id, k_oo_idx_temp);
    ...
    const order_line::key k_ol_1(warehouse_id, districtID, o_id,
                                 numeric_limits<int32_t>::max());
    tbl_order_line(warehouse_id)
        ->scan(txn, Encode(obj_key0, k_ol_0), &Encode(obj_key1, k_ol_1),
               c_order_line, s_arena.get());
...
STATIC_COUNTER_DECL(scopedperf::tod_ctr, stock_level_probe0_tod,
                    stock_level_probe0_cg)
STATIC_COUNTER_DECL(scopedperf::tod_ctr, stock_level_probe1_tod,
                    stock_level_probe1_cg)
STATIC_COUNTER_DECL(scopedperf::tod_ctr, stock_level_probe2_tod,
                    stock_level_probe2_cg)

static event_avg_counter
    evt_avg_stock_level_loop_join_lookups("stock_level_loop_join_lookups");

tpcc_worker::txn_result tpcc_worker::txn_stock_level() {
  const uint warehouse_id =
      PickWarehouseId(r, warehouse_id_start, warehouse_id_end);
  ...
  //    void *txn = db->new_txn(BenchmarkConfig::getInstance().getTxnFlags(),
  //    arena, txn_buf(), abstract_db::HINT_DEFAULT); const stock::key
  //    k_s(warehouse_id, i); const size_t nbytesread = serializer<int16_t,
  //    true>::max_nbytes(); auto ret=tbl_stock(warehouse_id)->get(txn,
  //    EncodeK(obj_key0, k_s), obj_v, nbytesread); if (!ret){
  //      cnt++;
  ...
  const uint64_t read_only_mask =
      g_disable_read_only_scans ? 0 : transaction_base::TXN_FLAG_READ_ONLY;
  const abstract_db::TxnProfileHint hint =
      g_disable_read_only_scans ? abstract_db::HINT_TPCC_STOCK_LEVEL
  ...
  try {
    ...
    const order_line::key k_ol_1(warehouse_id, districtID, cur_next_o_id, 0);
    {
      ...
      tbl_order_line(warehouse_id)
          ->scan(txn, Encode(obj_key0, k_ol_0), &Encode(obj_key1, k_ol_1), c,
                 s_arena.get());
    }
    {
      ...
      for (auto &p : c.s_i_ids) {
        ...
        INVARIANT(p.first >= 1 && p.first <= NumItems());
        {
          ...
          auto ret = tbl_stock(warehouse_id)
                         ->get(txn, EncodeK(obj_key0, k_s), obj_v, nbytesread);
          if (TThread::transget_without_stable) {
            TThread::transget_without_stable = false;
          }
          if (TThread::transget_without_throw) {
          ...
          if (!ret) {
            Warning("ERROR warehouse_id:%d, cid:%d, maxbytes:%d", warehouse_id,
                    p.first, nbytesread);
          }
          ALWAYS_ERROR(ret);
        }
      ...
      // NB(stephentu): s_i_ids_distinct.size() is the computed result of this
      // txn
    }
...
template <typename T> static vector<T> unique_filter(const vector<T> &v) {
  set<T> seen;
  vector<T> ret;
  for (auto &e : v)
    if (!seen.count(e)) {
...
private:
  ...
  static bool UseHashtable(const char *name) {
#if !HASHTABLE
    return false;
#endif
    return strcmp("customer", name) == 0 ||
           // strcmp("district", name) == 0 ||
           strcmp("history", name) == 0 || strcmp("item", name) == 0 ||
           strcmp("oorder", name) == 0 || strcmp("stock", name) == 0 ||
  ...
  OpenTablesForTablespace(abstract_db *db, const char *name,
    ...
    if (g_enable_separate_tree_per_partition && !is_read_only) {
      if (NumWarehouses() <= BenchmarkConfig::getInstance().getNthreads()) {
        ...
        const unsigned nwhse_per_partition =
            NumWarehouses() / BenchmarkConfig::getInstance().getNthreads();
        for (size_t partid = 0;
             partid < BenchmarkConfig::getInstance().getNthreads(); partid++) {
          const unsigned wstart = partid * nwhse_per_partition;
          const unsigned wend =
              (partid + 1 == BenchmarkConfig::getInstance().getNthreads())
                  ? NumWarehouses()
  ...
  static vector<abstract_ordered_index *>
  OpenTablesForTablespaceRemote(abstract_db *db, const char *name,
                                size_t expected_size) {
    const bool is_read_only = IsTableReadOnly(name);
    const bool is_append_only = IsTableAppendOnly(name);
    const bool use_hashtable = UseHashtable(name);
    ...
    const auto &local_shards =
        config ? config->local_shard_indices : std::vector<int>();
    ...
    for (size_t i = 0; i < NumWarehousesTotal(); i++) {
      ...
      if (s_idx == BenchmarkConfig::getInstance().getShardIndex()) {
        // Local to current shard - do nothing
                 ...
                 std::find(local_shards.begin(), local_shards.end(), s_idx) !=
                     local_shards.end()) {
        ...
        Notice("Remote table %s for wid %d: target shard %d is local, skipping "
               "RPC proxy",
               name, global_wid, s_idx);
      } else { // create remote tables for truly remote shards (different
               // process)
        abstract_ordered_index *idx =
            db->open_index(s_name + "_remote_" + to_string(global_wid), s_idx);
        ret[i] = idx;
      }
    }
    return ret;
...
public:
  tpcc_bench_runner(abstract_db *db, bool failure = false) : bench_runner(db) {
...
private:
  void init_tables(abstract_db *db, bool failure) {
    if (failure) {
      ...
      string nCount[12] = {"customer",        "customer_name_idx", "district",
                           "history",         "new_order",         "oorder",
                           "oorder_c_id_idx", "order_line",        "stock",
                           "stock_data",      "warehouse",         "item"};
      int table_id = 0;
      for (int i = 0; i < 12; i++) {
        if (nCount[i] != "item") {
          vector<abstract_ordered_index *> ret(
              BenchmarkConfig::getInstance().getNthreads());
          for (int j = 0; j < BenchmarkConfig::getInstance().getNthreads();
               j++) {
            table_id += 1;
...
#undef REMOTE_OPEN_TABLESPACE_X
    ...
    for (auto &t : partitions) {
      ...
      for (size_t i = 0; i < v.size(); i++) {
        ...
        // std::cout<<"Table: "<<t.first + "_" + to_string(i)<<",
        // id:"<<v[i]->get_table_id()<<std::endl;
      }
    }

    if (g_enable_partition_locks) {
      static_assert(sizeof(aligned_padded_elem<spinlock>) == CACHELINE_SIZE,
                    "xx");
      auto &cfg = BenchmarkConfig::getInstance();
      void *const px =
...
protected:
  virtual vector<bench_loader *> make_loaders() {
    ...
    if (BenchmarkConfig::getInstance().getEnableParallelLoading()) {
      ...
      for (uint i = 1; i <= NumWarehouses(); i++)
        ret.push_back(new tpcc_stock_loader(r.next(), db, open_tables,
                                            partitions, remote_partitions, i,
                                            shard_idx));
    } else {
      ret.push_back(new tpcc_stock_loader(89785943, db, open_tables, partitions,
                                          remote_partitions, -1, shard_idx));
    }
    ret.push_back(new tpcc_district_loader(
        129856349, db, open_tables, partitions, remote_partitions, shard_idx));
    if (BenchmarkConfig::getInstance().getEnableParallelLoading()) {
      fast_random r(923587856425);
      for (uint i = 1; i <= NumWarehouses(); i++)
        ret.push_back(new tpcc_customer_loader(r.next(), db, open_tables,
                                               partitions, remote_partitions, i,
                                               shard_idx));
    } else {
      ret.push_back(new tpcc_customer_loader(923587856425, db, open_tables,
                                             partitions, remote_partitions, -1,
                                             shard_idx));
    }
    if (BenchmarkConfig::getInstance().getEnableParallelLoading()) {
      fast_random r(2343352);
      for (uint i = 1; i <= NumWarehouses(); i++)
        ret.push_back(new tpcc_order_loader(r.next(), db, open_tables,
                                            partitions, remote_partitions, i,
                                            shard_idx));
    } else {
      ret.push_back(new tpcc_order_loader(2343352, db, open_tables, partitions,
                                          remote_partitions, -1, shard_idx));
    }
#endif
    return ret;
  ...
  virtual vector<bench_worker *> make_workers() {
    ...
    const int blockstart = coreid::allocate_contiguous_aligned_block(
        BenchmarkConfig::getInstance().getNthreads(), alignment);
    ...
    if (NumWarehouses() <= cfg.getNthreads()) {
      for (size_t i = 0; i < cfg.getNthreads(); i++) {
        ret.push_back(new tpcc_worker(
            blockstart + i, r.next(), db, open_tables, partitions,
            remote_partitions, &barrier_a, &barrier_b,
      ...
      for (size_t i = 0; i < cfg.getNthreads(); i++) {
        ...
        const unsigned wend = (i + 1 == cfg.getNthreads())
                                  ? NumWarehouses()
                                  : (i + 1) * nwhse_per_partition;
        ret.push_back(new tpcc_worker(
            blockstart + i, r.next(), db, open_tables, partitions,
            remote_partitions, &barrier_a, &barrier_b, wstart + 1, wend + 1,
            shard_index_)); // Pass shard index to worker
      }
    }
    return ret;
...
public:
  ...
  const map<string, vector<abstract_ordered_index *>> &get_partitions() const {
    return partitions;
  }
  const map<string, vector<abstract_ordered_index *>> &
  get_remote_partitions() const {
    return remote_partitions;
  }
  const map<string, abstract_ordered_index *> &get_open_tables_ref() const {
    return open_tables;
  }
  ...
  void wireup_cross_shard_tables(
      int source_shard,
      const map<string, vector<abstract_ordered_index *>> &source_partitions) {
    int num_warehouses = NumWarehouses();
    int start_wid_idx =
        source_shard * num_warehouses; // 0-indexed warehouse index
    ...
    for (auto &entry : remote_partitions) {
      ...
      if (it == source_partitions.end()) {
        Notice(
            "wireup_cross_shard_tables: table %s not found in source shard %d",
            table_name.c_str(), source_shard);
        continue;
      }
      ...
      for (int local_wid = 0;
           local_wid < num_warehouses && local_wid < (int)source_vec.size();
           local_wid++) {
        ...
        if (global_wid_idx < (int)remote_vec.size()) {
          ...
          // Notice("Wired up %s[%d] from shard %d local_wid %d",
          // table_name.c_str(), global_wid_idx, source_shard, local_wid);
        }
      }
    }
...
private:
  ...
  // remote_partitions has all partitions from other shards, and doesn't store
  // any data (for READ/WRITE)
  map<string, vector<abstract_ordered_index *>> remote_partitions;
  // queue holders moved into BenchmarkConfig
};

bench_runner *tpcc_do_test(abstract_db *db, int argc, char **argv, int run = 0,
  ...
  g_enable_separate_tree_per_partition =
      1; // keep it same as rolis, but in fact it doesn't matter
  while (1) {
    static struct option long_options[] = {
        {"disable-cross-partition-transactions", no_argument,
         &g_disable_xpartition_txn, 1},
        {"disable-read-only-snapshots", no_argument, &g_disable_read_only_scans,
         1},
    ...
    case 'r':
      ...
      ALWAYS_ERROR(g_new_order_remote_item_pct >= 0 &&
  ...
  // Override from BenchmarkConfig if set via dbtest.cc's
  // --new-order-remote-item-pct
  {
    auto &benchCfg = BenchmarkConfig::getInstance();
    int configPct = benchCfg.getNewOrderRemoteItemPct();
    if (configPct >= 0) {
      g_new_order_remote_item_pct = configPct;
      did_spec_remote_pct = true;
    }
  }
  ...
  if (BenchmarkConfig::getInstance().getVerbose()) {
    ...
    cerr << "  cross_partition_transactions : " << !g_disable_xpartition_txn
         << endl;
    cerr << "  read_only_snapshots          : " << !g_disable_read_only_scans
         << endl;
    cerr << "  partition_locks              : " << g_enable_partition_locks
         << endl;
...
bench_runner *tpcc_do_test(abstract_db *db, int argc, char **argv, int run,
                           bench_runner *rc, int shard_index) {
  if (run == 1) {
    ((tpcc_bench_runner *)rc)->run();
    mako::stop_erpc_server();
    return rc;
  ...
  // In multi-shard single-process mode, skip RPC setup since all shards are
  // local Cross-shard access will use local tables directly
  auto *config = BenchmarkConfig::getInstance().getConfig();
  bool multi_shard_mode = config && config->multi_shard_mode;
  if (!multi_shard_mode) {
    mako::setup_erpc_server();
    ...
    Notice(
        "Multi-shard mode: skipping RPC setup for shard %d (all shards local)",
        shard_index);
  }
  r->f_mode = f_mode;
  auto x1 = std::chrono::high_resolution_clock::now();
  printf(
      "start worker (shard %d):%d\n", shard_index,
      std::chrono::duration_cast<std::chrono::microseconds>(x1 - x0).count());
...
void wireup_cross_shard_tables_tpcc(bench_runner *target_runner,
                                    int source_shard_idx,
                                    bench_runner *source_runner) {
