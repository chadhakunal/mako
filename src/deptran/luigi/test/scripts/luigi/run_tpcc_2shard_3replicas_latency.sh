#!/bin/bash
# Wrapper script for 2shard-3replica TPCC benchmark with network latency and OWD/headroom
# Usage: ./run_tpcc_2shard_3replicas_latency.sh <duration> <threads> <owd_ms> <headroom_ms> <netem_delay_ms> <netem_jitter_ms>
...
echo "=== 2-Shard 3-Replica TPC-C Benchmark ==="
echo "Config: $CONFIG"
...
echo "Cleaning up..."
sudo tc qdisc del dev lo root 2>/dev/null || true
kill $S0_PID $S1_PID $S2_PID $S3_PID $S4_PID $S5_PID 2>/dev/null || true
pkill -9 luigi_server 2>/dev/null || true
