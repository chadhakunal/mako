...
Config::Config(char *ctrl_hostname, uint32_t ctrl_port, uint32_t ctrl_timeout,
    ...
    : heart_beat_(heart_beat), ctrl_hostname_(ctrl_hostname),
      ctrl_port_(ctrl_port), ctrl_timeout_(ctrl_timeout), ctrl_key_(ctrl_key),

      ctrl_init_(ctrl_init), tot_req_num_(tot_req_num), duration_(duration),
      config_paths_(vector<string>()), tx_proto_(0), proc_id_(0), benchmark_(0),
      scale_factor_(1), txn_weight_(vector<double>()),
      txn_weights_(map<string, double>()), proc_name_(string()),
      exp_setting_name_(string()), batch_start_(false), early_return_(false),
      retry_wait_(false), logging_path_(logging_path),
      single_server_(single_server), n_concurrent_(n_concurrent), max_retry_(1),
      num_site_(0), start_coordinator_id_(0), site_(vector<string>()),
      site_threads_(vector<uint32_t>()), num_coordinator_threads_(1), sid_(1),
      cid_(1), next_site_id_(0), proc_host_map_(map<string, string>()),
      sharding_(nullptr),
...
void Config::LoadSiteYML(YAML::Node config) {
  ...
  // Note: Using deque for sites_ so no reserve needed - push_back doesn't
  // invalidate pointers
...
void Config::LoadModeYML(YAML::Node config) {
  ...
  if (config["jetpack_recovery_batch_size"]) {
    jetpack_recovery_batch_size_ =
        config["jetpack_recovery_batch_size"].as<int>();
  }
}
...
void Config::UpdateWeights(YAML::Node config) {
  ...
  for (std::map<std::string, double>::iterator it = txn_weights_.begin();
       it != txn_weights_.end(); ++it) {
...
void Config::LoadShardingYML(YAML::Node config) {
  ...
  for (auto it = config.begin(); it != config.end(); it++) {
    ...
    for (auto replica_group_it = this->replica_groups_.begin();
         replica_group_it != this->replica_groups_.end(); replica_group_it++) {
...
std::optional<Config::SiteInfo>
Config::TryGetLeaderSiteByPartitionId(parid_t partition_id) {
  for (SiteInfo &site : sites_) {
    if (site.partition_id_ == partition_id && site.role == 0) {
      return site;
    }
  }
  return std::nullopt; // Not found - safe return for multi-process mode
}
...
vector<Config::SiteInfo> Config::SitesByProcessName(string proc_name,
  ...
  // Log_info("SitesByProcessName proc_name=%s type=%d", proc_name.c_str(),
  // type==SERVER);
  std::vector<SiteInfo> result;
